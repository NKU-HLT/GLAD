...
- **激活专家数量**：我们在三专家设置下进一步进行了实验，将路由 top-k 从 1 变化到 3。 我们发现，当选择全部三个专家（top-k = 3）时取得了最佳性能。这与我们的motivation一致。
  - 我们将这一现象归因于多说话人语音的内在复杂性。在较小的 top-k 设置下，并且在负载均衡loss的约束=下，学习信号被迫分配到有限的专家子集中，这可能导致单个专家缺乏充分的专门化能力。结果是，每个专家只能接收到部分信息且较为碎片化的表示，从而限制了其对复杂重叠语音条件的建模能力。
